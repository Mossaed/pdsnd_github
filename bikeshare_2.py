...
def prompt(df):
    ...
    """
    x = 0
    answer = input("nWould you like to see the first 5 rows? Press any key or type no:\n").lower()
    while True:
        if answer == 'no':
            break
        print(df[x:x+5])
        answer = input("nWould you like to see next 5 rows? Press any key or type no:\n").lower()
        x += 5
